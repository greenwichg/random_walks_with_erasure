...
#: Below this it is a Search/Discover source, which is exactly what Tier B is for.
...
def verdict(s: dict) -> "tuple[str, str]":
    """``(verdict, reason)`` for one outlet. Ordered so the disqualifying reasons are read first —
    a syndicator that also participates heavily is the WORST case, not a mixed one, because its
    participation is other publishers' coverage counted twice."""
    ...
    if s["participation"] < PARTICIPATION_FLOOR:
        return "TIER B", (f"{s['participation']:.0%} of its articles reach a story — a Search/Discover "
                          f"source, which is what Tier B is for")
    if s["rated"]:
    ...
    return "CURATE", (f"{s['participation']:.0%} participation, {s['syndication']:.0%} syndication, "
                      f"but UNRATED — worth a registry row so it can eventually vote")
...
def main(argv=None) -> int:
    ...
    for key, s in ranked[:args.show]:
        ...
        print(f"  {s['articles']:>6} {s['inStory']:>6} {s['participation']:>5.0%} "
              f"{s['syndication']:>5.0%} {s['hostStability']:>5.0%} {s['dated']:>5.0%}  "
              f"{s['canonical'][:30]:<30} {v}")

    # ---------------------------------------------------------------- verdict census
    census = Counter(verdict(s)[0] for s in cand.values())
    print(f"\n=== verdicts ===")
    for name, n in census.most_common():
        arts = sum(s["articles"] for s in cand.values() if verdict(s)[0] == name)
        print(f"  {n:>5} outlets  {arts:>7,} articles  {name}")

    demote = {k for k, s in cand.items() if verdict(s)[0] == "TIER B"}
    print(f"\n=== the reasons, for the {len(demote)} Tier B verdicts (read these) ===")
    for key in sorted(demote, key=lambda k: -cand[k]["inStory"])[:args.show]:
        print(f"  {cand[key]['canonical'][:36]:<36} {verdict(cand[key])[1]}")
