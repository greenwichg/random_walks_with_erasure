...
| `prune-backups.sh [--dry-run]` | **Tiered backup retention** (hourly/daily/weekly/monthly GFS) replacing the flat "keep newest N". Newest is always kept; unparseable names are never deleted. | Hourly cron at :33 (installed by `bootstrap-ec2.sh`). See `docs/STORAGE_LIFECYCLE.md`. |
