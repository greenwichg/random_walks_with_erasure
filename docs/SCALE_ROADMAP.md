...
| **M14** | **Language-targeted admission** — international stories are blocked by corpus DENSITY per language, not by the tokenizer. The fixed tokenizer reached only 3.0% of structurally-excluded articles, because a Korean headline with tokens still needs a Korean PEER inside the six-day window: 139 ko articles across 4 outlets, 98 ar across 6, 52 ja across 3. `source_discovery` ranks candidates by volume, which is dominated by English, so a volume-ordered expansion adds English to a corpus that already clusters English | M11, M13 | **`docs/SOURCE_PIPELINE_50K_AUDIT.md` §8.8.** This is what a 50,000-source INTERNATIONAL corpus actually requires; without it the extra sources cannot form stories with each other |
