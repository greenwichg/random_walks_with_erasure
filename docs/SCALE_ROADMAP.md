...
| M4 | Tier B story attachment by assignment + the byte-identical containment test | M1, M2 | this is what makes Tier B visible in stories. **Built 2026-08-30**: `story_service.attach_tier_b` — the shadow harness's own rule (`source_evaluation.would_attach`) run as a post-pass AFTER `stabilize_ids` syncs the member table, so an attached article is coverage on the wire (`tierB: true` tail + `attachedCoverage`) and never a member, vote, or id input. Flag `RWE_STORY_TIER_B_ATTACH`, dark by default; containment pinned by `tests/test_tier_b_attachment.py` (prefix property + member-table check, mutation-verified) |
...
| **M10** | **Discovery reads the catalogue, not the clustering window** — Stage 1 calls `story_service._fetch`, whose window is the 6-day clustering span, so it yields **177 candidates** from a 150,076-row catalogue; plus a host-observation table so evidence outlives article retention | M7 | **`docs/SOURCE_PIPELINE_50K_AUDIT.md`.** Supply is the first wall: past ~180 sources there is nothing left to admit. **Shipped and verified on production 2026-08-27: candidates go 177 → 1,173 (6.63×), against a ≥800 bar** |
| **M11** | **Source admission becomes durable, resumable data** — `store.SourceAdmission` replaces `crawler_publishers.json` (baked into the image) and the shadow half of the tier environment variables; states `candidate → probing → validated / rejected / incomplete → admitted / withdrawn`, per-host claims, and a `probe_count` that makes "never re-probed" measurable rather than asserted | M10 (nothing to admit at scale before it), M8, M9 | the only *join* in the chain with no mechanism; binds at 100–1,000. Subsumes M3's D6 for the shadow lane. **Built 2026-08-27**: `source_admission.py` (policy), `source_campaign.py` (runner), 40 tests, 12/12 mutations caught |
| **M13** | **The tokenizer excludes most of the world** — `title_tokens` matches `[a-z0-9]+`, so ko/ar/zh/ja/ru/ta/hi headlines yield ZERO tokens and `pair_admits` rejects them before any other test: they cannot join a story under any configuration. Measured 0.6% participation against 29% for English. Accented Latin fragments rather than fails (`kündigt` → `ndigt`), which also splits ENGLISH stories over `Erdoğan`/`Erdogan` | M1 | **`docs/SOURCE_PIPELINE_50K_AUDIT.md` §8.** A 50k corpus is necessarily international, and a source that cannot join a story contributes nothing to coverage or to a blindspot claim. **MEASURED AND REJECTED**: rescued 78, cost 149 (1.9x), reached 3.0% of the excluded population, Vietnamese coverage 32 -> 0. `--unicode-fallback` is the indicated variant (zero cost by construction), unmeasured. **The real finding is M14** |
| **M14** | **Language-targeted admission** — international stories are blocked by corpus DENSITY per language, not by the tokenizer. The fixed tokenizer reached only 3.0% of structurally-excluded articles, because a Korean headline with tokens still needs a Korean PEER inside the six-day window: 139 ko articles across 4 outlets, 98 ar across 6, 52 ja across 3. `source_discovery` ranks candidates by volume, which is dominated by English, so a volume-ordered expansion adds English to a corpus that already clusters English | M11, M13 | **Designed: `docs/M14_LANGUAGE_DENSITY_DESIGN.md`.** The design's own finding is that Tier A is bounded at ~2x today's volume (~5,000 more long-tail sources before the 60,000 row cap), so M14 is an ALLOCATION policy for a scarce resource, not an admission-volume problem. Two blockers it surfaces: M8's `assignment_rate` scores 0% by construction for every candidate in a zero-coverage language, and Tier A promotion needs a lean nobody has for ko/ar/ja. **STOPPED 2026-08-28 on a pre-registered bar.** Stage 0 ran on the corrected peer axis and the peer hypothesis failed with 15 monotonicity breaks: `nl` has ZERO publishers above the 10-article floor and the 3rd-highest density, 6x Spanish with 153 publishers. Neither publisher count nor article count predicts density — see §13. What survives: the unlabelled quarter (~16% of the window invisible to targeting) and the validated instrument |
