...
|---|---|---|
| `GET /v1/health` | — | liveness, the versions in force, enrichment coverage (entities / spans / event geography over the catalogue and the last 7 days; counted in the background, `null` until the first count after a start), `lastBuildAt`, `history` (the story recorder's row counts, the last completed build's counters, the last error since start), search-index status |
