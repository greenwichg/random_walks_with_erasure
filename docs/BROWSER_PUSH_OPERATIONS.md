...
| A reader says push stopped after they re-enabled it | — | Fixed in the subscription-lifecycle commit. SQLite reuses row ids, so a device recreated after a `410` prune used to inherit the pruned one's ledger rows and be skipped for anything the old device received. A fresh registration now discards them. The cost of that discard is the row two below — the two are the same trade seen from each side, and this is the side worth paying for: a repeat is recoverable, silence is not. |
...
| Recent notifications arrive again right after a device re-registers | `push_subscription_created` immediately before the run | **Expected, and the deliberate direction of an unavoidable trade.** A fresh registration discards the ledger rows carrying its `subscription_id` (the row above), so anything still inside its expiry window is unclaimed and therefore deliverable again. The `tag` collapses each repeat onto the existing notification rather than stacking a duplicate. Bounded by the events' own `expires_at`, not by the ledger — a re-registration cannot resurrect anything already stale. |
