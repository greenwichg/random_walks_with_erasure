...
- [ ] **Browser push:** leave `RWE_PUSH_ENABLED` **unset or `0`** for go-live. Phase B1 registers subscriptions but sends nothing, so enabling it would put a permission prompt in front of readers in exchange for no notifications. All four `RWE_PUSH_*` / `RWE_VAPID_*` variables are declared on the **`api`** service in both compose files (`deploy/ops/validate-deployment.py` fails if the switch is missing, or if the keys are missing while it is on). Ref: `docs/BROWSER_PUSH_OPERATIONS.md`.
