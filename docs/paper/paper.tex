...
validates} ($\mathrm{lean\_corr}=0.65$, cleanly ideological extremes) once
low-signal subreddits are filtered---giving an independent RQ3 on a validated axis
where RWE-B again bridges hardest. The flagship MIND RQ3 still rests on the weak
...
$\mathrm{lean\_corr}=0.65$, with cleanly ideological extremes---communism/anarchism/
...
step) and rests on $n{=}20$ labels and a single seed, so we report it as validated
but small-sample. On this validated axis RQ3 holds: \textbf{RWE-B again bridges
...
($\mathrm{lean\_corr}{=}0.65$, $n{=}20$ labels, single seed) and are directional, not
...
\begin{enumerate}
  ...
  (Spearman~$\approx0.27$ vs.\ human labels, and $\approx0$ on a second blind
  40-headline set---it conflates \emph{topic} with \emph{stance}); (ii)~a
  ...
  ($\mathrm{lean\_corr}=0.65$ against labeled subreddit leans, cleanly ideological
  extremes). The lesson is itself a finding: news co-clicks and headlines come out
  topical, but a left--right axis \emph{is} recoverable from explicit
  ...
  and rests on $n{=}20$ labels and a single seed. \textbf{So the flagship MIND RQ3
  (text-lean axis) is \emph{suggestive}}, but the bounded-bridging \emph{mechanism}
