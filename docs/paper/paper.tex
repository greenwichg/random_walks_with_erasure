...
\begin{enumerate}
  ...
  prototyped in simulation but also \textbf{closed on a real measured signal} on
  Politosphere (Table~\ref{tab:loop}), and that signal is empirically motivated: on $14.7$\,M Politosphere
...
\paragraph{Closing the loop on real reception.} The satisfaction-calibrated
controller of \S\ref{sec:method} (\texttt{AdaptiveRWEB}) is \emph{defined} on a
simulated browsing walk; Politosphere lets us instead \emph{drive} it from a
\emph{measured} per-user signal. We set each user's exposure---hence their bridging
$\varepsilon$---from their \emph{hardened} cross-cutting reception
(\texttt{cross\_welcomed\_frac}: a comment counts as welcomed only if it was upvoted
\emph{and} not flagged controversial, so a comment the user's \emph{own} side merely
vote-brigades up is not read as a welcome). On the validated-axis graph ($15$K served
users, $52\%$ carrying a signal), against a uniform RWE-B at the \emph{same average dose}
the measured-adaptive policy \emph{redistributes} rank-weighted opposite-content reach
toward the readers whose real bridges were tolerated (Table~\ref{tab:loop}): adaptive
reach rises monotonically across tolerance terciles ($0.35\!\to\!0.76\!\to\!1.29$) while
the uniform policy does not track tolerance ($0.51/0.75/0.74$), giving low-tolerance
readers a \emph{gentler} dose and high-tolerance readers a \emph{stronger} one
($\mathrm{Spearman}(\text{tolerance},\text{reach}){=}{+}0.66$). The signal is
self-selected (only crossover users carry it---an upper bound) and coarse per user, so we
report this as a \emph{mechanism demonstration on real data}, not a settled satisfaction
gain.

\begin{table}[t]
  \centering
  \caption{Closing the loop on \emph{real} Politosphere reception. At the same average
  dose, \texttt{AdaptiveRWEB} driven by \emph{measured} tolerance redistributes
  rank-weighted opposite-content reach (top-10) toward readers whose real bridges were
  welcomed, while a uniform policy does not ($\mathrm{Spearman}{=}{+}0.66$);
  $n{\approx}1{,}334$ per tercile. Self-selected signal, so a mechanism demonstration,
  not a satisfaction-gain claim.}
  \label{tab:loop}
  \small
  \begin{tabular}{lcc}
    \toprule
    ...
    \midrule
    low\ \,(0.00--0.29)  & \textbf{0.35} & 0.51 \\
    mid\ \,(0.29--0.71)  & 0.76 & 0.75 \\
    high (0.71--1.00)    & \textbf{1.29} & 0.74 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  articles agree on the exact L/C/R label only at Cohen's $\kappa{=}0.14$ while
  flipping Left$\leftrightarrow$Right just $2.2\%$ of the time
  ($\kappa_{\mathrm{side}}{=}0.58$), i.e.\ the \emph{per-article} label is unreliable
  though its \emph{direction} is moderately stable---usable only in aggregate; and on
  \textbf{human AllSides gold} (Qbias, $n{=}3{,}000$) text-lean is a \emph{weak,
  model-sensitive} proxy---politicalBiasBERT is near-chance at every length
  ($\kappa{=}0.007$ headline, $0.001$ body), a second AllSides model (premsa) reaches only
  Spearman~$0.22$ ($\kappa_{\mathrm{side}}{=}0.30$) with the body, and both are dwarfed by an
  \textbf{outlet lookup} on the same gold (Spearman~$0.918$, $\kappa{=}0.84$,
  $\kappa_{\mathrm{side}}{=}1.00$---${\approx}4\times$ the best text model): the lean lives far
  more in the \emph{publisher} than the words, motivating an outlet-first axis); (ii)~a
...
\paragraph{Future work.} Each limitation above has a concrete resolution path. The weak
text-lean axis is the clearest: on the same AllSides gold an \emph{outlet} lookup recovers the
label at $\kappa{=}0.84$ where even the best of two text classifiers reaches only Spearman~$0.22$, so an outlet-first
axis---already demonstrated on Qbias---is the fix wherever publishers are recoverable (MIND's
are gated behind MSN URLs, but most corpora ship them). The behavioral axis would firm up with
more than $n{=}20$ labeled communities, and the ideological result would generalize with a
second news corpus beyond US-2019. The reception and depolarization claims are self-selected and
simulation-based by necessity; only a deployment that recommends bridges to \emph{random} users
and measures real acceptance can replace the observational upper bound with a causal
estimate---the natural next step, at which the synthetic-user pipeline is swapped for real
behaviour. The remaining constraints (MSN gating, the original private Twitter data, BERT's
512-token context, and data-redistribution licences) are external, handled with public proxies
and documented work-arounds.
