...
are explicit about limitations: \textbf{none of three independent axis
constructions} (text-lean, news co-click, Reddit behavioral ideal point)
\textbf{validated as ideological}, so RQ3 is suggestive and the contribution is the
bounded-bridging \emph{mechanism} (robust on any 1-D axis); the centre-ward effect
is also partly geometric.
\end{abstract}
...
\paragraph{Third dataset, and a negative result on the axis.} We additionally ran
RQ1 on the \textbf{Reddit Politosphere}~\cite{politosphere2022}, where the
left--right axis is learned from \emph{behaviour}---an ideal-point fit on the
user$\times$subreddit endorsement graph (15{,}000 users, 295 political subreddits,
US-2016 window). RWE-D is again the top long-tail diversifier at accuracy parity
with $RP^3_\beta$ (AUC .942, HR@10 .637; gini .189, coverage .993, avg-deg 1594,
surprisal 4.048), so the long-tail result holds on \textbf{three independent public
datasets} (news, movies, social). \emph{But the behavioral axis came out
non-ideological}: validated against 24 labeled subreddit leans, $\mathrm{lean\_corr}=0.13$
($n{=}24$, indistinguishable from 0), with extremes that are ideologically scrambled
(e.g.\ \texttt{r/secondamendment} and \texttt{r/Marco\_Rubio} grouped \emph{with}
\texttt{r/anarchocommunism}). The dominant dimension an unsupervised ideal-point fit
recovers from broad Reddit commenting is niche-subreddit idiosyncrasy, not
left--right, so we report Politosphere for RQ1 only and \emph{not} for ideological
bridging. This is the third independent axis construction to come up unvalidated
(\S\ref{sec:ethics}, Limitation~1).
...
\label{sec:ethics}
\begin{enumerate}
  \item \textbf{No validated ideological axis (three attempts).} Recovering a
  trustworthy left--right axis from public data proved genuinely hard. We tried
  (i)~a \textbf{text-lean} classifier (Spearman~$\approx0.27$ vs.\ human labels, and
  $\approx0$ on a second blind 40-headline set---it conflates \emph{topic} with
  \emph{stance}); (ii)~a \textbf{MIND co-click} ideal point ($r=0.37$, topical); and
  (iii)~a \textbf{Reddit Politosphere behavioral} ideal point built to fix this,
  which came out \emph{non-ideological} ($\mathrm{lean\_corr}=0.13$, scrambled
  extremes). The convergent failure is itself a finding: news co-clicks are topical,
  broad Reddit commenting is cross-cutting, and headlines conflate topic with stance;
  the cleanest remaining signal is Twitter elite-\emph{following} (Barberá-style),
  now access-restricted. \textbf{So RQ3 is explicitly \emph{suggestive}}, and the
  contribution is the bounded-bridging \emph{mechanism} (Limitation~2, robust on any
