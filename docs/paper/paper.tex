...
\begin{enumerate}
  ...
  \textbf{human AllSides gold} (Qbias, $n{=}3{,}000$) the classifier lands at near-chance
  from the \emph{text at every length we tested}---$\kappa{=}0.007$ from the headline and
  $\kappa{=}0.001$ from its 256-token opening, collapsing to \emph{centre} both ways---whereas an
  \textbf{outlet lookup} recovers the same gold at
  $\kappa{=}0.84$ ($\kappa_{\mathrm{side}}{=}1.00$)---the lean lives in the \emph{publisher},
  not the words, motivating an outlet-first axis); (ii)~a
...
label at $\kappa{=}0.84$ where the text classifier is near-chance, so an outlet-first
