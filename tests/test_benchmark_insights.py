...
def test_report_contains_the_sections_the_reader_needs(server, articles):
    runs = [bench.run_target(_target("good", "fine-model", server), articles, repeats=1),
            bench.run_target(_target("bad", "bad-model", server), articles, repeats=1),
            bench.run_target(bench.Target(name="hosted", provider="anthropic"), articles, 1)]
    md = bench.report_markdown([bench.summarize(r) for r in runs], runs, articles,
                               repeats=1, note="unit test")
    for needle in ("## Results", "pass rate", "## Failure breakdown", "## Samples",
                   "## Method & caveats", "unit test", "tokens in/out",
                   "est. cost / 1k articles"):
