...
def test_notebook_defaults_are_safe_and_offline():
    ...
    # offline: no web app / server / tunnel like the playground, and no matplotlib (D4)
    assert "npm " not in text and "cloudflared" not in text and "next-server" not in text
    assert "matplotlib" not in text and "pyplot" not in text
    # installs the engine library (D1)
