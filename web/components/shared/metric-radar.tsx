...
import type { MetricRadar as Impl } from "./metric-radar-impl";

/**
 * Code-splitting boundary for MetricRadar.
 *
 ...
 */
const Lazy = dynamic(() => import("./metric-radar-impl").then((m) => m.MetricRadar), {
  ssr: false,
  loading: () => <div aria-hidden className="w-full animate-pulse rounded-lg bg-muted/40" />,
});

export function MetricRadar(props: React.ComponentProps<typeof Impl>) {
  return (
    <div style={{ minHeight: props.size ?? 320 }}>
      <Lazy {...props} />
    </div>
  );
}
